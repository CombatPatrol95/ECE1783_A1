% Define the video frame parameters
width = 352;     % Width of the frame
height = 288;    % Height of the frame
frameSize = width * height * 1.5; % Size of one frame in bytes

% Open the input YUV file for reading
fileID = fopen('foreman_cif-1.yuv', 'rb');


yuvData = fread(fileID, inf, 'uint8');
fclose(fileID);

% Set up variables for saving Y-only frames
frameNumber = 0;
...
while feof(fileID)
    % Read a frame (YUV 4:2:0 format)
    frame = fread(fileID, frameSize, 'uint8');

    % Separate the Y component
    Y = frame(1:width * height);

    % Create a file name for the Y-only frame
    yOnlyFileName = ['y_only_files\frame', num2str(frameNumber), '.y'];

    % Save the Y component as a Y-only file
    yOnlyFileID = fopen(yOnlyFileName, 'wb');
    fwrite(yOnlyFileID, Y, 'uint8');
    fclose(yOnlyFileID);
...
for frameId = 0:length(frameNumber)-1
    % Create a file name for the Y-only frame
    yOnlyFileName = ['y_only_files\frame', num2str(frameId), '.y'];
    fileID = fopen(yOnlyFileName, 'rb');
    frame = fread(fileID, width * height, 'uint8');
    
    for iId = 1:length(blockSizes)
        i = blockSizes(iId);
        % Check if padding is needed
        if mod(width, i) > 0 || mod(height, i) > 0
            % Calculate the number of rows and columns to add for padding
            padRows = i - mod(height, i);
            padCols = i - mod(width, i);

            % Pad the frame with gray (128)
            frame = padarray(frame, [padRows, padCols], 128, 'post');
        end
    end

    frameSize = size(frame,1);
    % Assume a hypothetical reconstructed reference frame with all samples as 128
    referenceFrame = 128 * ones(size(width * height));
    ...
    for blockX = 1:numBlocksX
        for blockY = 1:numBlocksY
            ...
            currentBlock = frame((blockY-1)*i*width + (blockX-1)*i + 1 : blockY*i*width + blockX*i);

            % Perform motion estimation using integer pixel full search
            bestMAE = Inf;
            bestMV = [0, 0];

            for rIdx = 1:length(searchRanges)
                r = searchRanges(rIdx);
                for mvY = -r:r
                    for mvX = -r:r
